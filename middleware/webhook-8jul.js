...
export const createPrediction = async(message, roomId, chatId, file = null) => {
    ...
    try {
        ...
        if (file && file.url) {
            // ✅ ALWAY SEND MULTIMODAL PAYLOAD if there’s file
            payload = {
                question: [{
                    role: "user",
                    content: [{
...
export const receiveMessage = async(req, res) => {
    ...
    try {
        ...
        // console.log("BODY Qontak:", JSON.stringify(body, null, 2));

        // === KIRIM LANGSUNG JIKA ADA FILE ===
        if (file) {
            // console.log("Langsung kirim ke worker karena ada file:", file);
            // console.log("=== Payload yang akan dikirim ke worker ===", JSON.stringify({
