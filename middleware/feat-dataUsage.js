...
export const receiveMessage = async(req, res) => {
    ...
    try {
        ...
        const room_id = body && body.room_id ? body.room_id : (room && room.id ? room.id : "");
        ...
        if (body && body.type === "text") {
            ...
            if (body.file && body.file.url) {
                let filename = body.file.filename || "file";
                let mimetype = "";
        ...
        // --- NEW FEATURE: check ICCID & OrderId ---
        let flowiseContext = {};
        const parsed = parseIccidAndOrderId(message);
        ...
        messageBuffers[room_id].timeout = setTimeout(async() => {
